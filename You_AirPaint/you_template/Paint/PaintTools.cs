...
namespace You_AirPaint.Paint
{
    /// <summary>
    /// Methods to paint the Bitmap Image
    /// </summary>
    ...
    public static class PaintTools
    {
        ...
        /// <summary>
        /// Represents a line, and optimizes the calculation of its nearest distance to multiple points
        /// </summary>
        ...
        static PaintTools()
        {
            AirbrushBytes = new byte[2 * AirbrushRadio + 1][];
            
            for(int i = 0;i < 2 * AirbrushRadio + 1;i++)
              AirbrushBytes[i] = new byte[2 * AirbrushRadio + 1];
            ...
            for(int i = 0;i < AirbrushBytes.Length;i++)
            {
                ...
                {
                    y = j - AirbrushRadio;
                    AirbrushBytes[i][j] = (byte)(128 - (int)(128.0 * Math.Min(1, Math.Sqrt(x * x + y * y) / (double)AirbrushRadio)));
                }
            }
        }
